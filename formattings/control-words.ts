...
export const CONTROL_WORD_REPLACEMENTS: ControlWordEntry[] = [
  ...
  { commands: ['nächstes darunter eingerückt'], pattern: /[.,;\s]*\bnächstes\s*darunter\s*eingerückt\b[.,;\s]*/gi, replacement: '\n  - ' },
  { commands: ['nächstes eingerückt'], pattern: /[.,;\s]*\bnächstes\s*eingerückt\b[.,;\s]*/gi, replacement: '\n  - ' },
  { commands: ['nächstes darunter'], pattern: /[.,;\s]*\bnächstes\s*darunter\b[.,;\s]*/gi, replacement: '\n- ' },
  // "nächstes" alleinstehend wird NUR ersetzt, wenn vorher eine der obigen Langformen
  // vorkam (bedingte Ersetzung in applyFormattingControlWordsWithStats).
  ...
  { commands: ['Komma', 'Beistrich'],      pattern: /\bkomma\b/gi, replacement: ',' },
  { commands: ['Komma', 'Beistrich'],      pattern: /\bbeistrich\b/gi, replacement: ',' },
