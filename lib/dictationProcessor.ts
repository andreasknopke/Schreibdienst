...
/** Current processing state (read-only for status endpoint) */
export function getWorkerState() {
  return { isProcessing, lastProcessTime };
}

/**
 * Process all pending dictations (up to `limit`).
 * Returns immediately if already processing.
 */
export async function processAllPending(request: NextRequest, limit = 5): Promise<{ processed: number; errors: number }> {
  ...
  try {
    ...
    for (const dictation of pending) {
      try {
        await processDictation(request, dictation.id);
        processed++;
      } catch (error: any) {
...
async function doublePrecisionMerge(
  ...
  if (!merged.hasDifferences) {
    ...
    console.log(
      `[Worker DoublePrecision] No unresolved differences found, using merged transcription ` +
    ...
    return { text: sourceText, segments: result1.segments };
  }
  ...
  // fusionieren kann (z. B. "Histologie" + "R004998-26" → "Histologier-26").
